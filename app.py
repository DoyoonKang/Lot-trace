...
# Config
# =========================
st.set_page_config(page_title="액상 잉크 Lot 추적", layout="wide")
...
def get_binder_limits(spec_binder: pd.DataFrame, binder_name: str):
    # Spec_Binder: rows per binder + 시험항목, 하한, 상한
...
def infer_binder_type_from_lot(spec_binder: pd.DataFrame, binder_lot: str):
    """
    binder lot prefix (PCB/BIH/SBCT...) -> binder name (PBHAv1.0/PBHBv1.0/SBCT...)
    by matching Spec_Binder Lot부여규칙.
    """
    if not binder_lot:
        return None
    rules = (
    ...
    for r in rules:
        ...
        if m:
            ...
            if str(binder_lot).startswith(prefix):
                return r["바인더명"]
    # fallback: SBCT rule sometimes is SBCT+YYYYMMDD
    return None


def next_seq_for_pattern(existing_lots: pd.Series, prefix: str, date_str: str, digits: int = 2, sep: str = "-"):
    """
    Find next sequence for lots like PREFIXYYYYMMDD-## or PREFIXYYMMDD##, depending on sep.
    """
...
def generate_binder_lot(spec_binder: pd.DataFrame, binder_name: str, mfg_date: dt.date, existing_binder_lots: pd.Series):
    ...
    # parse: PCB+YYYYMMDD-## or SBCT+YYYYMMDD
...
def generate_single_lot(single_df: pd.DataFrame, product_code: str, color_group: str, in_date: dt.date):
    # Rule: PL/NPL + colorcode + YYMMDD + 2-digit seq  OR SL + colorcode + YYMMDD + 2-digit seq
    code = (product_code or "").strip()
    color_code = COLOR_CODE.get(color_group)
    if not color_code:
        return None

    # Decide prefix (PL/NPL/SL)
    if code.startswith("NPL"):
        ...
        # fallback based on ink type: if it looks like silicone code
        prefix = "PL"
...
def append_row_to_sheet(xlsx_path: str, sheet_name: str, row: dict):
    """
    Append a row dict to a sheet. Column order follows the existing header row (row 1).
    Unknown keys are ignored; missing keys become blank.
    """
...
st.caption("✅ 빠른 검색 + ✅ 신규 입력(엑셀에 누적) + ✅ 기본 대시보드")

# Data file selection
with st.sidebar:
    st.header("데이터 파일")
    xlsx_path = st.text_input("엑셀 파일 경로", value=DEFAULT_XLSX, help="로컬 실행 시, app.py와 같은 폴더에 엑셀을 두면 기본값 그대로 사용 가능합니다.")
...
# normalize date columns for display / filtering
...
with tab_input:
    ...
    with sub_s:
        ...
        # binder lot candidates from binder_df
        ...
        if submit_s:
            # infer binder type from lot prefix + spec
            binder_type = infer_binder_type_from_lot(spec_binder, binder_lot)

            # viscosity spec from Spec_Single_H&S
...
with tab_dash:
    ...
    # Charts (viscosity trend)
    left, right = st.columns([2, 1])

    with left:
        st.subheader("단일색 점도 분포 (색상군별)")
